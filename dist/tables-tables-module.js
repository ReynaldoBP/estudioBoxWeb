...
/* harmony import */ var app_services_reporte_service__WEBPACK_IMPORTED_MODULE_8__ = __webpack_require__(/*! app/_services/reporte.service */ "./src/app/_services/reporte.service.ts");
var __decorate = (undefined && undefined.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
...
var PublicacionesListComponent = /** @class */ (function () {
    function PublicacionesListComponent(objEncuestaService, objExportarDataService, toastr, objSucursalService, reporteService, objAreaService) {
    ...
    PublicacionesListComponent.prototype.descargarRespuestasGeneral = function (objCltEncuesta) {
        var _this = this;
        this.objLoading = true;
    ...
    PublicacionesListComponent.prototype.getCargarDataParaReporte = function () {
        ...
        var empresasPermitidas = [11, 14, 18];
        ...
        if (!empresasPermitidas.includes(this.user.intIdUsuarioEmpresa)) {
            ...
            this.objEncuestaService.consultarInformacionAdicional(this.arrayParametrosInfoAdicional).subscribe(function (data) {
                _this.objLoading = false;
                if (data["intStatus"] == 200) {
                    var arrayAreaCaract = data['arrayAreaCaract'];
                    console.log(arrayAreaCaract);
                    for (var _i = 0, arrayAreaCaract_1 = arrayAreaCaract; _i < arrayAreaCaract_1.length; _i++) {
                        var valor = arrayAreaCaract_1[_i];
                        ...
                        if (valor.strCaracteristica == "NUMERO_PACIENTE") {
                            document.getElementById('pacientes').value = valor.strValor1;
                        }
                        if (valor.strCaracteristica == "FACTURAS VALIDAS") {
                            document.getElementById('facturaValida').value = valor.strValor1;
                        }
                        if (valor.strCaracteristica == "ENCUESTA FÍSICA") {
                            document.getElementById('encuestaFisica').value = valor.strValor1;
                        }
                        if (valor.strCaracteristica == "NO CONTESTO") {
                            document.getElementById('noContesto').value = valor.strValor1;
                        }
                        if (valor.strCaracteristica == "MÍNIMO A OBTENER") {
                            document.getElementById('minObtener').value = valor.strValor1;
                        }
                    }
                }
                else {
                    _this.toastr.warning('No Existen datos con los parámetros por favor ingresar.', 'Informativo');
                }
            }, function (error) {
                _this.toastr.warning("Error en el servidor, comuniquise con el dpto. de sistemas");
            });
        ...
        var formHTML = (this.user.intIdUsuarioEmpresa == 11 || this.user.intIdUsuarioEmpresa == 18) ? this.formHTMLHospital : this.formHTMLTenisClub;
        ...
        sweetalert2__WEBPACK_IMPORTED_MODULE_4___default()({
            ...
            width: 600,
            preConfirm: function () {
                if (_this.user.intIdUsuarioEmpresa == 11 || _this.user.intIdUsuarioEmpresa == 18) {
                    var pacientes = document.getElementById('pacientes').value;
                    if (!pacientes || isNaN(Number(pacientes))) {
                        sweetalert2__WEBPACK_IMPORTED_MODULE_4___default.a.showValidationMessage("Debe ingresar un número de pacientes válido");
                    }
                    _this.arrayParametrosInfoAdicional.pacientes = Number(pacientes);
                }
                else if (_this.user.intIdUsuarioEmpresa == 14 || _this.user.intIdUsuarioEmpresa == 14) {
    ...
    PublicacionesListComponent.prototype.procesarInformacionAdicional = function (arrayParametrosInfoAdicional) {
        ...
        this.objEncuestaService.ingresarInformacionAdicional(arrayParametrosInfoAdicional).subscribe(function (data) {
            _this.objLoading = false;
            if (data["intStatus"] == 200) {
                sweetalert2__WEBPACK_IMPORTED_MODULE_4___default()({ title: "Datos Adicionales", text: "Datos Adicionales ingresado exitosamente!", type: "success", showConfirmButton: true })
                    .then(function (result) {
                    if (result.value)
                        _this.getDataEncuesta();
                });
    ...
    PublicacionesListComponent.prototype.getExportarCsvPersonalizado = function () {
        var _this = this;
        this.objLoading = true;
        var arrayParametrosReporteEncuesta = {
            intIdUsuario: this.user.intIdUsuario,
        ...
        this.reporteService.getReporteDataEncuesta(arrayParametrosReporteEncuesta).subscribe(function (blob) {
            _this.objLoading = false;
    ...
    PublicacionesListComponent = __decorate([
        ...
        __metadata("design:paramtypes", [app_services_encuesta_service__WEBPACK_IMPORTED_MODULE_5__["EncuestaService"],
            ...
            app_services_reporte_service__WEBPACK_IMPORTED_MODULE_8__["ReporteService"],
