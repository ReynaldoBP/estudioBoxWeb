    ...
    this.objSucursalService.getSucursal(this.objParametrosSucursal)
      .subscribe(
        data => {
          ...
          const sucursalesAsignadas = this.arraySucursal.filter(sucursal => sucursal.intIdUsSucursal !== null).map(sucursal => sucursal.intIdSucursal);
          // Asignar las sucursales preseleccionadas al modelo de usuario
          this.usuario.arrayIdSucursal = sucursalesAsignadas;
          this.getArea()
        },
        error => {
          this.toastr.warning('Hubo un error, por favor comuníquese con el departamento de sistemas.', 'Error')
    ...
    this.objAreaService.getArea(this.arrayParametrosArea)
      .subscribe(
        data => {
          ...
          const areasAsignadas = this.arrayArea.filter(area => area.intIdUsArea !== null).map(area => area.intIdArea);
          // Asignar las áreas preseleccionadas al modelo de usuario
          this.usuario.arrayIdArea = areasAsignadas;
        },
        error => {
          this.toastr.warning('Hubo un error, por favor comuníquese con el departamento de sistemas.', 'Error')
        }
